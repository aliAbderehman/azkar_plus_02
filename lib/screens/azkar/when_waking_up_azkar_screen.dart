...
import 'package:azkar_plus/screens/azkar/contents/azkar_after_waking_up.dart';
...
class WhenWakingUpAzkarScreen extends StatefulWidget {
  const WhenWakingUpAzkarScreen({super.key});

  @override
  State<WhenWakingUpAzkarScreen> createState() =>
      _WhenWakingUpAzkarScreenState();
}
...
class _WhenWakingUpAzkarScreenState extends State<WhenWakingUpAzkarScreen> {
  ...
  void updateZikr(String sens, bool isVibrate) {
    ...
    if (sens == '+') {
      if (index == whenWakingUpAzkar.length - 1) {
        if (azkIndex == whenWakingUpAzkar[index].counter) {
          index = 0;
      ...
      body: Column(
        children: [
          Row(
            ...
            children: [
              ...
              StatText(
                azkIndex: azkIndex,
                index: index,
                counter: whenWakingUpAzkar[index].counter,
              ),
              Expanded(
                child: Row(
          ...
          Expanded(
            flex: 4,
            child: AzkarCard(
              azkarAr: whenWakingUpAzkar[index].arabicAzkar,
              azkarAm: TranslatePrefs.getTranslateStatus()!
                  ? whenWakingUpAzkar[index].amharicTranslation
                  : '',
              counter: (index),
              totalAzkar: whenWakingUpAzkar.length,
              subCounter: azkIndex,
            ),
          ),
          SizedBox(
            ...
            child: Row(
              ...
              children: [
                ...
                SmallButton(
                  onPressed: () {
                    setState(() {
                      azkIndex = whenWakingUpAzkar[index].counter;
                      updateZikr('+', isVibrate);
                    });
                    azkIndex = 1;
                  },
